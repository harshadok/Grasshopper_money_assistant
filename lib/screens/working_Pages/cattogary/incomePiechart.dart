...
class IncomeCatogary extends StatefulWidget {
  const IncomeCatogary({Key? key}) : super(key: key);

  @override
  State<IncomeCatogary> createState() => _CatogryOverViewState();
}

class _CatogryOverViewState extends State<IncomeCatogary> {
  ...
  Future<void> filteFuction() async {
    ...
    await Future.forEach(list, (TranscationModel obj) {
      if (obj.date.day == monthYear.day &&
          obj.catogaryModel.type == CatogaryType.icome) {
